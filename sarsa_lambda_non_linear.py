...
def test_algorithm(num_tiles,num_tilings):
    ...
    a = np.arange(0.2, 0.81, 0.1)
    points = [[0.2, 0.5], [0.5, 0.2], [0.5, 0.8], [0.8, 0.5]]
    ...
    def eval_critical_points(w):
        ...
        for x in a:
            for y in a:
                points.append([x,y])
        rect = patches.Rectangle((0.15,0.15),0.7,0.7,linewidth=1,edgecolor='r',facecolor='none')
        ax.add_patch(rect)

        for s in points:
            Q = [np.dot(w, X(s,False,a)) for a in range(env.num_actions)]
            ...
            if (best_action ==0):
                ...
                rep = 'up'
            plt.text(s[0], s[1], str(rep), horizontalalignment='center',verticalalignment='center')
            plt.xlim((0,1))
            plt.ylim((0,1))
        plt.show()

    def _eval(w, s, render=False):
        # print(s)
        s, done = env.reset(s), False
        ...
        return G
    perf_average = []
    eval_critical_points(w_final)
    for i in range(w_array.shape[0]):
        w_i = w_array[i]
        # print(",".join(map(str, w_i)))

        Gs = [_eval(w_i, s) for s in  points]
...
if __name__ == "__main__":
    ...
    tiles_list = range(4,5)
    for num_tilings  in range(1,2):
        for num_tiles in tiles_list:
            np.random.seed(num_tiles)
            results = test_algorithm(num_tiles, num_tilings)
            average.append(results[1:])
            ax.plot(np.arange(len(results)),results, label=(str(num_tilings) + " Tiling with "  + str(num_tiles)+ " Tiles" ))
